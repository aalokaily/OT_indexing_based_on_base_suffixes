...
Suffix trees are fundamental and efficient data structures for solving string-related problems. A suffix tree is a compressed trie that stores all suffixes of a string of length $n$ over an alphabet of size $\Sigma$, and it can be constructed in linear time, i.e., $O(n)$. In this work, we present an algorithm that constructs a practical and near-linear index, which enables finding pattern occurrences under any internal node in $O(\log_2 n + occ)$ time, where $occ$ denotes the number of occurrences of the pattern under the internal node. This index further facilitates approximate pattern matching (with $k$ Hamming distance as an instance) in $O(\frac{m^k}{k!}\log_2 n + occ)$ time (and in $O(\frac{\log_\Sigma^k n}{k!}\log_2 n + occ)$ on average if the suffix tree is balanced), where $m$ is the length of the pattern and $k$ is the number of tolerated errors.
